...
for repo in "$source_dir" "$source_dir/paper-api" "$source_dir/paper-server"; do
    if [[ -e "$repo/.git" ]]; then
        git -C "$repo" config user.name "FoliaCarpet Builder"
        git -C "$repo" config user.email "build@localhost"
    fi
done
